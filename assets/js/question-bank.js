...
"color": "var(--c-ts)",
"level": "intermediate",
"q": "You are forecasting store sales and have foot-traffic data. How should you use it?",
"a": "<p>The deciding question is whether the covariate is known in advance at the time you make the forecast. Foot traffic is observed, not scheduled, so at an origin of today forecasting next week you will not have next week's value. Using it contemporaneously produces an excellent backtest and a model that cannot run in production. Three options are legitimate. Lag the covariate far enough that the value you need is always available at the origin, which is usually the right answer. Forecast the covariate first and accept that its error propagates, which means your evaluation must use the forecast rather than the actual. Or drop it in favour of covariates that genuinely are known ahead, such as holidays, planned promotions and published prices. The Temporal Fusion Transformer makes this distinction explicit by taking separate inputs for known-future and past-only covariates, which is a useful discipline even if you never use that model.</p>"
},
{
"id": "17-time-series-forecasting::4",
"pageId": "17-time-series-forecasting",
"page": "17-time-series-forecasting",
"pageTitle": "Time Series & Forecasting",
"group": "Decision & Retrieval Systems",
"color": "var(--c-ts)",
"level": "intermediate",
"q": "Why do gradient-boosted trees struggle with trending series, and what do you do about it?",
"a": "<p>A tree predicts the mean of the training examples in a leaf, so its output is bounded by the range of targets it saw during training. Outside the range of feature values it observed it simply returns the nearest edge value and stays flat. On a series with a persistent upward trend, every forecast beyond the training range is therefore biased low, and the bias grows with the horizon, which usually shows up as a forecast curve that goes suspiciously flat. The remedy is to remove the trend before the tree sees it: difference the target and have the model predict the change rather than the level, or divide by a rolling baseline and model the ratio, then invert the transform on the output. An alternative is to fit a simple linear or seasonal component first and let the tree model the residual. This is also the main structural reason the classical toolkit still matters, since ARIMA and ETS extrapolate trends by construction.</p>"
},
{
"id": "17-time-series-forecasting::5",
"pageId": "17-time-series-forecasting",
"page": "17-time-series-forecasting",
"pageTitle": "Time Series & Forecasting",
"group": "Decision & Retrieval Systems",
...
"color": "var(--c-graph)",
"level": "beginner",
"q": "Walk me through one layer of a graph neural network.",
"a": "<p>Every node holds a feature vector, and a layer updates all of them at once in three steps. First gather: each node reads the current vectors of the nodes it is connected to. Second aggregate: those neighbour vectors are combined into a single message by a function that must be permutation invariant, because a node's neighbours have no ordering and no fixed count, which rules out concatenation and leaves sum, mean, max and attention. Third update: the message is combined with the node's own vector and passed through a shared learned transform and a nonlinearity. Written as matrices this is one product of a normalised adjacency matrix with the feature matrix and a weight matrix. Stacking k layers means each node's output depends on its k-hop neighbourhood, so depth buys receptive field exactly as it does in a convolutional network.</p>"
},
{
"id": "18-graph-ml::1",
"pageId": "18-graph-ml",
...
"color": "var(--c-graph)",
"level": "beginner",
"q": "Why not flatten the adjacency matrix and feed it to an MLP?",
"a": "<p>Two reasons, and both are fatal. First, an MLP over a flattened adjacency matrix is not permutation invariant: relabelling the nodes of the same graph produces a different input and therefore a different prediction, even though nothing about the graph changed. The model would have to learn invariance from data, which wastes enormous capacity on a property you could have built in. Second, the input size is fixed, so a network trained on graphs with a thousand nodes cannot accept one with a thousand and one, and real graphs vary in size constantly. Message passing solves both structurally. It is defined per node in terms of an unordered neighbour set, so it gives the same answer under relabelling and accepts any graph size with the same weights.</p>"
},
{
"id": "18-graph-ml::2",
"pageId": "18-graph-ml",
...
"color": "var(--c-graph)",
"level": "intermediate",
"q": "Compare GCN, GraphSAGE and GAT.",
"a": "<p>They are the same three-step loop with different aggregation. GCN weights each neighbour by one over the square root of the product of the two degrees, a fixed quantity computed from the graph before training, and adds self-loops so a node keeps its own state. GraphSAGE changes two things: it samples a fixed number of neighbours per layer rather than using all of them, which bounds compute regardless of hub degree, and it concatenates the node's own vector with the aggregated message instead of mixing it in, so the self representation has its own weights. It is also explicitly inductive, learning a function of features rather than an embedding per node. GAT replaces the fixed coefficient with a learned attention weight per edge, computed from both endpoints' features and normalised by a softmax over the neighbourhood, which is self-attention with the adjacency matrix as the mask. GCN is the cheapest, GraphSAGE is what scales, GAT helps when neighbours differ in relevance.</p>"
},
{
"id": "18-graph-ml::3",
"pageId": "18-graph-ml",
...
"color": "var(--c-graph)",
"level": "intermediate",
"q": "What is the difference between transductive and inductive learning on graphs, and how do you split data for each?",
"a": "<p>Transductive means the entire graph, including all edges and all node features, is available during training and only some labels are hidden, so you split the labels rather than the graph. Most citation benchmarks are set up this way, which is why their numbers look strong. Inductive means the test nodes or graphs were genuinely unseen at training time, either because they are separate graphs as in molecular datasets or because the graph was cut at a timestamp. Production is almost always inductive, since the users and transactions you will score tomorrow are not in tonight's snapshot. The splitting rules follow: for node classification hide labels but keep the structure in the transductive case, and hold out whole subgraphs or a time range in the inductive case. For link prediction the held-out edges must be removed from the adjacency used for message passing, including their reverse direction in an undirected graph, or the model simply reads the target off its own input.</p>"
},
{
"id": "18-graph-ml::4",
"pageId": "18-graph-ml",
...
"color": "var(--c-graph)",
"level": "intermediate",
"q": "Why do deep GNNs usually perform worse than shallow ones?",
"a": "<p>Because a message-passing layer is a low-pass filter, and stacking them is a power iteration. Each layer averages every node with its neighbours, which pulls connected representations closer, and repeated application converges toward the dominant eigenvector of the propagation matrix, so every node in a connected component ends up with essentially the same vector. The rate is set by the second eigenvalue, and on a small well-connected graph the pairwise distance between node representations can fall by an order of magnitude within ten or so layers. A classifier then has nothing left to separate. This is over-smoothing, and it is why most deployed GNNs are two or three layers deep. Mitigations keep earlier representations reachable: residual connections, jumping knowledge that concatenates every layer's output, normalisation schemes such as PairNorm, and decoupling propagation from transformation as SGC and APPNP do, with APPNP's restart term permanently retaining a share of each node's own features.</p>"
},
{
"id": "18-graph-ml::5",
"pageId": "18-graph-ml",
...
"color": "var(--c-graph)",
"level": "deep",
"q": "How would you train a GNN on a graph with a billion edges?",
"a": "<p>You stop treating a minibatch as a set of rows and start treating it as a subgraph, because with average degree thirty and three layers a single target node already pulls in tens of thousands of others. Three families of sampling exist. Node-wise sampling, from GraphSAGE, fixes a fan-out per layer such as twenty-five then ten, which is simple and still the common default though it remains exponential in depth. Layer-wise sampling, as in FastGCN, draws a fixed number of nodes per layer with importance weights so total cost is linear in depth, at the price of needing care that sampled layers share enough edges. Subgraph sampling, as in Cluster-GCN and GraphSAINT, partitions the graph and runs full-batch on one partition, which is cache-friendly but discards edges that cross partition boundaries. Beyond sampling you cap the degree of hub nodes so one celebrity does not blow up a batch, and you decide early whether serving will do live k-hop lookups or precompute embeddings nightly, since that choice constrains the architecture more than accuracy does.</p>"
},
{
"id": "18-graph-ml::6",
"pageId": "18-graph-ml",
...
"color": "var(--c-graph)",
"level": "deep",
"q": "Distinguish over-smoothing from over-squashing.",
"a": "<p>Both present as adding layers making results worse, but they are opposite problems. Over-smoothing is too much mixing: repeated neighbourhood averaging is a low-pass filter that drives node representations together until they are indistinguishable, and it gets worse with depth by construction. Over-squashing is too little bandwidth: the number of nodes within distance k grows exponentially while the fixed-width vector carrying their information does not, so a message from a distant node must funnel through a small number of bottleneck edges and arrives degraded. Depth does not fix squashing, because the constraint is the topology rather than the number of layers. The diagnosis differs too. If the mean pairwise distance between node representations is collapsing, it is smoothing, and the fixes are residuals, jumping knowledge or decoupled propagation. If the representations stay well spread but long-range tasks fail, it is squashing, and the fixes change the graph rather than the network: rewiring to shorten paths, adding a virtual node connected to everything, or replacing local propagation with global attention.</p>"
},
{
"id": "18-graph-ml::7",
"pageId": "18-graph-ml",
...
"color": "var(--c-graph)",
"level": "deep",
"q": "You are asked to detect fraud rings in a payment network. How do you approach it?",
"a": "<p>Start by checking whether the graph is worth using at all, with a gradient-boosted tree on node features as the baseline the GNN must beat, because that comparison is frequently lost and is cheap to run. Then check homophily, since fraud is often heterophilous: a mule connects to victims rather than to other mules, and plain neighbourhood averaging erases the contrast, which argues for architectures that keep self and neighbour representations separate, such as GraphSAGE with concatenation, or explicitly heterophily-aware layers. The model must be inductive, because accounts are created continuously and a transductive embedding table has no row for them, and it should be temporal, since the ordering and recency of transactions carry most of the signal. Train with neighbour sampling and cap hub degree so a high-volume merchant does not dominate a batch. Evaluate on a time-based split with precision at a realistic alert budget rather than AUC, since labels are extremely imbalanced and delayed. Finally, plan serving up front: a k-hop lookup inside a latency budget, or nightly precomputed embeddings served as features, which is a genuine freshness tradeoff against an adversary who adapts in days.</p>"
},
{
"id": "19-scientific-structured-ai::0",
"pageId": "19-scientific-structured-ai",
"page": "19-scientific-structured-ai",
"pageTitle": "Scientific & Structured AI",
"group": "Embodied & Frontier",
"color": "var(--c-sci)",
"level": "beginner",
"q": "What is the difference between invariance and equivariance, and why does it matter?",
"a": "<p>A function is invariant to a transformation if applying the transformation to the input leaves the output unchanged, and equivariant if the output undergoes the corresponding transformation. Rotating a molecule leaves its energy unchanged, so energy is a rotation-invariant quantity, while the force on each atom rotates along with the molecule, so forces are equivariant. The distinction decides what a model is capable of emitting. A network built on pairwise distances discards the orientation entirely, which makes it exactly invariant but means it can only ever produce scalars, since it has no representation of direction. To predict a vector quantity you need equivariant internal features that carry orientation through the layers. Getting this wrong shows up as a model that cannot express the target at all, rather than as a model that trains badly.</p>"
},
{
"id": "19-scientific-structured-ai::1",
"pageId": "19-scientific-structured-ai",
"page": "19-scientific-structured-ai",
"pageTitle": "Scientific & Structured AI",
"group": "Embodied & Frontier",
...
"color": "var(--c-sci)",
"level": "intermediate",
"q": "Walk through how AlphaFold 2 gets from a sequence to coordinates.",
"a": "<p>The input is not a single sequence but a multiple sequence alignment of evolutionary relatives, plus any structural templates, because residue pairs that mutate together are likely to be in physical contact and that coevolution signal carries much of the prior. The Evoformer trunk maintains two coupled representations, one over the alignment and one over residue pairs, and passes information between them for many blocks. Triangle multiplicative updates constrain each pair edge using the other two edges of every triangle, imposing the triangle inequality that real distances obey. The structure module then gives each residue a local frame and uses Invariant Point Attention to reason about points in those frames, making it equivariant to global rotation and translation, and it outputs backbone frames and torsion angles. The network is recycled on its own output several times, trained end to end against a frame-aligned point error, and emits a per-residue pLDDT confidence.</p>"
},
{
"id": "19-scientific-structured-ai::4",
"pageId": "19-scientific-structured-ai",
"page": "19-scientific-structured-ai",
"pageTitle": "Scientific & Structured AI",
"group": "Embodied & Frontier",
...
"color": "var(--c-world)",
"level": "intermediate",
"q": "Why does Dreamer learn behaviours in latent space rather than in pixel space?",
"a": "<p>Three reasons, all of them practical. First, cost: an imagined step in a compact latent is a small matrix operation, whereas an imagined step in pixel space means generating an image, and Dreamer needs enormous numbers of imagined steps to train an actor and a critic. Second, fit: a latent dynamics model has orders of magnitude fewer parameters than a pixel-space predictor, so it can be fitted from the small amount of real data that made the whole approach worthwhile. Third, and most importantly, predictability: the encoder is trained so that the latent is something the dynamics can actually predict, while raw pixels contain a great deal of high-frequency detail that is both unpredictable and irrelevant to control. A pixel loss forces capacity onto texture and noise. The decoder exists to supply a training signal for the representation, and planning never needs to call it.</p>"
},
{
"id": "22-world-models::5",
"pageId": "22-world-models",
"page": "22-world-models",
"pageTitle": "World Models",
"group": "Embodied & Frontier",
...
"color": "var(--c-atlas)",
"level": "beginner",
"q": "In a mixture-of-experts model, why do total and active parameters both matter?",
"a": "<p>Total parameters set the memory requirement, because every expert must be resident even though only a few run per token. Active parameters set the compute and memory traffic of a single decode step, so they determine speed and arithmetic cost. A model with 2.8 trillion total and 104 billion active parameters decodes at roughly the cost of a 104-billion dense model but needs the memory footprint of a 2.8-trillion one. That combination is excellent economics on a large served cluster and unusable on a single workstation, which is why quoting only one of the two numbers is misleading.</p>"
},
{
"id": "33-model-atlas-language::2",
"pageId": "33-model-atlas-language",
"page": "33-model-atlas-language",
"pageTitle": "Model Atlas: Language Models",
"group": "Model Atlas",
"color": "var(--c-atlas)",
"level": "intermediate",
"q": "A vendor reports 90% on a benchmark and an independent harness reports 78%. What explains the gap?",
"a": "<p>Several things, and usually more than one at once. The vendor may run at a higher reasoning-effort setting, which is a different price and latency point on the same weights. The scaffold differs: retry policy, tool definitions, context management and output parsing can move agentic scores by double digits. Prompt formatting and few-shot examples differ. The evaluation harness may parse answers more strictly, counting near-misses as failures. None of these imply dishonesty, but they do mean a vendor number is not directly comparable to a third-party number, and the decision should rest on a run you control.</p>"
},
{
"id": "33-model-atlas-language::3",
"pageId": "33-model-atlas-language",
"page": "33-model-atlas-language",
"pageTitle": "Model Atlas: Language Models",
"group": "Model Atlas",
...
"color": "var(--c-atlas)",
"level": "intermediate",
"q": "Why is a single AIME score a weak signal?",
"a": "<p>Because the contest has only fifteen problems, so each one is worth about 6.7 percentage points. Sampling noise alone can move a single-run score by more than the gap between two models being compared. Reasoning models are also stochastic, so repeated runs on the same weights disagree. A defensible AIME number is an average over many runs with a reported variance, and comparing two single runs tells you very little. The same argument applies with less force to any small benchmark, including GPQA Diamond.</p>"
},
{
"id": "33-model-atlas-language::5",
"pageId": "33-model-atlas-language",
"page": "33-model-atlas-language",
"pageTitle": "Model Atlas: Language Models",
"group": "Model Atlas",
...
"color": "var(--c-atlas)",
"level": "deep",
"q": "What does a one-million-token context window actually buy you?",
"a": "<p>Less than the number suggests. Retrieval of a single fact from deep in the window is now largely solved, but reasoning that requires combining many facts spread across the window degrades well before the advertised limit. Long contexts also cost real resources: prefill time grows with prompt length and directly sets time to first token, and the KV cache grows linearly with the context, so concurrency falls as context rises. In practice a retrieval step that puts five relevant pages in a short context usually beats putting five hundred pages in a long one, and it is cheaper on every axis. Test at the depth you intend to use before designing around the headline number.</p>"
},
{
"id": "34-model-atlas-speech-audio::0",
"pageId": "34-model-atlas-speech-audio",
"page": "34-model-atlas-speech-audio",
"pageTitle": "Model Atlas: Speech & Audio",
"group": "Model Atlas",
"color": "var(--c-atlas)",
"level": "beginner",
"q": "What is word error rate and what are its limitations?",
"a": "<p>Word error rate is the sum of substitutions, deletions and insertions divided by the number of words in the reference transcript, so it is an edit distance normalised by length. Because insertions are counted, it can exceed 100%. Its limitations are that it treats every word as equally important, when in practice names, numbers and product terms carry nearly all the value, and that it is highly sensitive to text normalisation choices such as casing, punctuation and number formatting. Two published figures for the same model can differ by several points purely from the datasets averaged and the normaliser applied.</p>"
},
{
"id": "34-model-atlas-speech-audio::1",
"pageId": "34-model-atlas-speech-audio",
"page": "34-model-atlas-speech-audio",
"pageTitle": "Model Atlas: Speech & Audio",
"group": "Model Atlas",
"color": "var(--c-atlas)",
"level": "beginner",
"q": "What is the difference between noise suppression, source separation and target speaker extraction?",
"a": "<p>Noise suppression takes one speaker plus non-speech noise and returns clean speech, so the output is a single stream and the model is usually small and causal for real-time use. Source separation takes a mixture of several sources and returns one stream per source, which is the framing used for splitting two overlapping speakers or splitting a song into instrument stems. Target speaker extraction takes a mixture plus a short enrolment clip of one person and returns only that person's voice, which sidesteps the problem of deciding how many sources are present. They use different metrics too: DNSMOS and PESQ for enhancement, SI-SDR improvement for separation.</p>"
},
{
"id": "34-model-atlas-speech-audio::2",
"pageId": "34-model-atlas-speech-audio",
"page": "34-model-atlas-speech-audio",
"pageTitle": "Model Atlas: Speech & Audio",
"group": "Model Atlas",
...
"color": "var(--c-atlas)",
"level": "intermediate",
"q": "Why do CTC and TDT decoders have such enormous throughput advantages over LLM decoders?",
"a": "<p>Because they are not autoregressive over text. A CTC decoder produces a frame-level output in a single pass and decodes greedily, so the cost is one encoder forward pass over the audio. An LLM decoder generates the transcript token by token, each step requiring a forward pass through a language model, which is the same memory-bandwidth-bound decode loop that limits text generation. The result is a throughput gap measured in one to two orders of magnitude, with Parakeet-class models reaching inverse real-time factors in the thousands against double digits for Whisper large-v3. The trade is accuracy, since the language model decoder repairs ambiguous audio using linguistic context that CTC has no mechanism to apply.</p>"
},
{
"id": "34-model-atlas-speech-audio::4",
"pageId": "34-model-atlas-speech-audio",
"page": "34-model-atlas-speech-audio",
"pageTitle": "Model Atlas: Speech & Audio",
"group": "Model Atlas",
